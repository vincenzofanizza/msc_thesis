...
def upload_speedplus_to_s3(speedplus_root, bucket_name):
    ...
    lightbox_path = os.path.join('speedplusv2', 'lightbox')

    lightbox_image_root = os.path.join(lightbox_path, 'images')
    lightbox_filenames = os.listdir(os.path.join(speedplus_root, lightbox_image_root))
    for image_filename, _ in zip(lightbox_filenames, tqdm(range(1, len(lightbox_filenames)), desc = 'uploading lightbox images')):
        upload_file_to_s3(filepath = os.path.join(speedplus_root, lightbox_image_root, image_filename),
    ...
    upload_file_to_s3(filepath = os.path.join(speedplus_root, lightbox_path, 'test.json'), 
                bucket_name = bucket_name, 
    ...
    sunlamp_path = os.path.join('speedplusv2', 'sunlamp')
    
    sunlamp_image_root = os.path.join(sunlamp_path, 'images')
    sunlamp_filenames = os.listdir(os.path.join(speedplus_root, sunlamp_image_root))
    for image_filename, _ in zip(sunlamp_filenames, tqdm(range(1, len(sunlamp_filenames)), desc = 'uploading sunlamp images')):
        upload_file_to_s3(filepath = os.path.join(speedplus_root, sunlamp_image_root, image_filename),
                        bucket_name = bucket_name, 
    ...
    upload_file_to_s3(filepath = os.path.join(speedplus_root, sunlamp_path, 'test.json'), 
                    bucket_name = bucket_name, 
    ...
    synthetic_path = os.path.join('speedplusv2', 'synthetic')
        
    synthetic_image_root = os.path.join(synthetic_path, 'images')
    synthetic_filenames = os.listdir(os.path.join(speedplus_root, synthetic_image_root))
    for image_filename, _ in zip(synthetic_filenames, tqdm(range(1, len(synthetic_filenames)), desc = 'uploading synthetic images')):
        upload_file_to_s3(filepath = os.path.join(speedplus_root, synthetic_image_root, image_filename),
                        bucket_name = bucket_name, 
    ...
    upload_file_to_s3(filepath = os.path.join(speedplus_root, synthetic_path, 'train.json'), 
                    bucket_name = bucket_name, 
                    key = os.path.join(synthetic_path, 'train.json').replace('\\', '/')
                    )
    upload_file_to_s3(filepath = os.path.join(speedplus_root, synthetic_path, 'validation.json'), 
                    bucket_name = bucket_name, 
    ...
    upload_file_to_s3(filepath = os.path.join(speedplus_root, 'speedplusv2', 'camera.json'), 
                    bucket_name = bucket_name, 
                    key = os.path.join('speedplusv2', 'camera.json').replace('\\', '/')
                    )
    upload_file_to_s3(filepath = os.path.join(speedplus_root, 'speedplusv2', 'LICENSE.md'), 
                    bucket_name = bucket_name, 
...
def save_image_to_s3(image, bucket_name, filepath):
    ...
    Args:
        image (numpy.array): Array containing the image in the RGB format.
        bucket (str): Name of the S3 bucket.
